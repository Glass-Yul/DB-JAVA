...
select employee_id 사번, first_name || last_name 이름 from employees order by employee_id desc;
select employee_id 사번, first_name || last_name 이름 from employees order by 이름;

---------------------------------------------------

--3.1.4 : 중복제거(distinct)
select distinct salary 급여 from employees order by salary;
---------------------------------------------------

--[예제 3-4] : job_id값을 중복제거
select distinct job_id id from employees;
--------------------------------------------------
...
select email||'@company.com' 이메일 from employees;
---------------------------------------------------

--3.1.7 : 산술연산 : (), *, /, _, -
--[예제 3-8] : 사원의 사원번호, 급여, 추가급여(급여+500),인하급여(급여-100), 조정급여(급여에 10%추가하여 2로 나눔) 조회 
select employee_id 사번, salary+500 추가급여, salary-100 인하급여, (salary*0.1/2) 조정급여 from employees;
---------------------------------------------------

--[예제 3-9] : 별명 사용하여 사원의 사원번호, 급여, 추가급여,인하급여, 조정급여 조회 
select employee_id 사번, salary+500 추가급여, salary-100 인하급여, (salary+salary*0.1)/2 조정급여, (salary*1.1)/2 조정급여2 from employees;
---------------------------------------------------
...
select * from employees where salary=1000 or salary=17000 or salary=240000;
--[예제 3-14] : IN연산자 사용 : 급여가 1000, 17000, 240000인 정보조회
select * from employees where salary in(1000 ,17000 ,240000);
---------------------------------------------------
