...
-- [예제 4-18] : 사원의 행의 수
select count(*)사원수 from employees;
---------------------------------------------------

-- [예제 4-19] : 전체 사원의 연봉 합계, 연봉 평균, 계산 평균연봉을 조회하시오.
-- 평균 = 전체급여합계/사원건수
select sum(salary) 연봉_합계, avg(salary) 연봉_평균, sum(salary)/count(*) 평균
from employees;
---------------------------------------------------

-- [예제 4-20] : 전체사원의 연봉 중 연봉 최대값, 연봉 최솟값, 사원의 이름중 최대문자값, 최소문자값을 조회하시오.
select max(salary), min(salary), max(length(first_name||last_name)) 최대문자값, min(length(first_name||last_name)) 최소문자값
from employees;
---------------------------------------------------
...
select job_id 직무별ID, sum(salary) 총연봉, avg(salary) 평균연봉
from employees
...
select job_id 직무별ID,manager_id 관리자ID, sum(salary) 총연봉, avg(salary) 평균연봉
from employees
where employee_id>=10
