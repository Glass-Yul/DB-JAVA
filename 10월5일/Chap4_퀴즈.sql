...
-------------------------------------------------------------------------------
select department_id 사원번호, first_name||last_name 이름, substr(job_id,1,2) 부서, job_id 직무,
salary 연봉, 
    case
...
-------------------------------------------------------------------------------
select first_name||last_name 이름, upper(job_id) 직무
from employees
where last_name like '%s';
-------------------------------------------------------------------------------

-- Quiz 4-3.
-- 모든 사원의 연봉을 표시하는 연봉 보고서를 작성하려 한다.
...
-------------------------------------------------------------------------------
select first_name||last_name 이름, salary + salary*NVL(commission_pct, 1) 연봉,
NVL2(commission_pct, 'salary Only', 'salary+Commition') 수당여부
from employees;
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
select first_name||last_name 이름, to_char(hire_date,'YYYY-MM-DD') 입사일,
...
-- DECODE 사용 --
select department_id 사번, first_name||last_name 이름, job_id 직무, salary 연봉,
decode(job_id,'HR_ERP',salary*1.1,'MK_ERP',salary*1.2,'PR_ERP',salary*1.5,
                'SA_ERP',salary*1.8,'IT_PROG',salary*2.1,salary) "인상된 연봉"
from employees;
...
-------------------------------------------------------------------------------
select department_id 사번, first_name||last_name 이름, hire_date 입사일자, salary 연봉,
case
